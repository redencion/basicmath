module Arithmetic
  class Operations
    ...
    def by_sign(operation)
      ...
      pattern_brackets = [/\(([0-9].*?)\)/,/\[([0-9].*?)\]/]
      pattern_brackets.each_with_index do |j,i|
        while operation.scan(j).size > 0  do 
          regex_result = operation[j]
          result_to_operation = operation[j,1]
